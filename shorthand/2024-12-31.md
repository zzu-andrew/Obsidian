...
import (  
    "go.uber.org/zap"  
    "go.uber.org/zap/zapcore"    "os"    "time")  
...
func main() {  
    ...
    encoderConfig.EncodeTime = zapcore.ISO8601TimeEncoder // 使用ISO8601格式化时间  
    encoder := zapcore.NewJSONEncoder(encoderConfig)      // 使用JSON格式  
  
    // 创建一个新的core，它会把日志写入到我们打开的文件中  
    ...
    defer logger.Sync() // 确保在程序结束时刷新所有缓冲的日志  
